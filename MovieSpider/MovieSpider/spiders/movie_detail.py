...
import json
import re
from MovieSpider.db_handler import cursor
...
class MovieDetailSpider(scrapy.Spider):
    ...
    # 近期热门电影
    recent_hot_movie_url = 'https://movie.douban.com/j/search_subjects?type=movie&tag=%E7%83%AD%E9%97%A8&sort=time&page_limit=50&page_start=0'

    # 爬虫入口
    def start_requests(self):
        return [scrapy.Request(self.recent_hot_movie_url, callback=self.handle_need_urls)]

    # 处理需要爬取的url
    def handle_need_urls(self, response):
        # 近期电影数据
        data_dict = json.loads(response.text)
        movie_list = data_dict['subjects']
        new_id_list = list(map(lambda x: int(x['id']), movie_list))
        # 获取原始数据
        sql = 'select douban_id from movie_detail'
        cursor.execute(sql)
        douban_id_tuple_list = cursor.fetchall()
        old_id_list = list(map(lambda x: x[0], douban_id_tuple_list))
        # 作差集获取数据库中没有，即需要爬取的电影id
        need_crawl_id_list = list(set(new_id_list) - set(old_id_list))
        print("需要抓取{}条电影数据".format(len(need_crawl_id_list)))
        # 生成需要爬取的电影链接
        need_crawl_url_list = ['https://movie.douban.com/subject/{}/'.format(i) for i in need_crawl_id_list]
        for url in need_crawl_url_list:
